...
const {
  ValidateInsertFormData,
  ValidateUpdateFormData,
} = require("../../controller/core/Account_con");
const {
...
router.post("/register", SetUserInformation, async (req, res, next) => {
  try {
    const { UserName, Password, ConfirmPassword, Email, Phone, Address, Role } =
      req.body;
    ...
    const { CustomerFirstName, CustomerMiddleName, CustomerLastName } =
      req.body; // Customer
    ...
    if (!ValidationResult.valid) {
      ...
      } else res.status(500).send("Error registering!");
    }
...
router.post("/login", SetUserInformation, async (req, res, next) => {
  try {
    ...
    const token = jwt.sign(
      ...
      process.env.LOGGING_JWT_SECRET,
      { expiresIn: "1h" }
    );
...
router.get("/update", VerifyTokenFromCookie, async (req, res) => {
  try {
    ...
    else res.status(500).send("Error retrieving user data: ");
  } catch (error) {
...
router.post("/update", VerifyTokenFromCookie, async (req, res, next) => {
  try {
    ...
    if (!ValidationResult.valid) {
      RoleData = UserData;
      return res.render("core/UserUpdate.ejs", {
        UserData,
        RoleData,
        ErrorMessage: ValidationResult.message,
